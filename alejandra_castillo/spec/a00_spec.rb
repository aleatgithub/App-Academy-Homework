...
describe "#digital_root" do
  before(:each) do
    ...
    expect_any_instance_of(Integer).to_not receive(:inspect)
  ...
  it "calculates the digital root of a larger number whose digits sum to an amount greater than 10" do
...
describe "#jumble_sort" do
  before(:each) do
    expect_any_instance_of(Array).to_not receive(:sort)
    expect_any_instance_of(Array).to_not receive(:sort_by)
  end
